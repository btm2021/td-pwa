...
function createATRBotVP(PineJS) {
    return {
        name: "ATR Bot VP",
        metainfo: {
            _metainfoVersion: 51,
            id: "atrbot_vp@tv-basicstudies-1",
            name: "ATR Bot VP",
            description: "ATR Dynamic Trail with EMA and State Change Detection",
            shortDescription: "ATR Bot VP",
        ...
        constructor: function () {
            ...
            this.main = function (context, inputCallback) {
                ...
                const vp_numRows = this._input(5);
                ...
                if (index > this.last_committed_index) {
                    if (this.pending_bar_data && this.pending_bar_index === index - 1) {
                        this.current_cycle_bars.push(this.pending_bar_data);
                    }
                    this.last_committed_index = index;
                }

                if (!isNaN(high) && !isNaN(low) && !isNaN(volume)) {
                    this.pending_bar_data = { high, low, volume };
                ...
                const display_bars = [...this.current_cycle_bars];
                if (this.pending_bar_data && this.pending_bar_index === index) {
                    display_bars.push(this.pending_bar_data);
                }

                let poc = NaN, vah = NaN, val = NaN;
                if (display_bars.length > 0) {
                    const stats = this.calculateFRVP(display_bars, vp_numRows, vp_vaPct);
                    poc = stats.poc;
                    vah = stats.vah;
                    val = stats.val;
                }

                // Output live changing FRVP of the CURRENT cycle.
                return [trail1, trail2, trail1_green, trail1_red, poc, vah, val];
            };
            this.calculateFRVP = function (bars, numRows, vaPct) {
                if (!bars || bars.length === 0) return { poc: NaN, vah: NaN, val: NaN };
                ...
                for (let i = 0; i < bars.length; i++) {
                    const b = bars[i];
                    if (!isNaN(b.low) && b.low < min) min = b.low;
                    if (!isNaN(b.high) && b.high > max) max = b.high;
                    if (!isNaN(b.volume)) totalVol += b.volume;
                }

                if (min === Infinity || max === -Infinity || Math.abs(max - min) < 1e-10 || totalVol === 0) {
                    return { poc: min, vah: max, val: min };
                }

                const priceRange = max - min;
                const rowSize = priceRange / numRows;
                const profile = new Array(numRows).fill(0);
                ...
                for (let i = 0; i < bars.length; i++) {
                    ...
                    if (startRow >= numRows) startRow = numRows - 1;
                    if (startRow < 0) startRow = 0;
                    if (endRow >= numRows) endRow = numRows - 1;
                    if (endRow < 0) endRow = 0;

                    if (b.high === b.low) {
                        profile[startRow] += b.volume;
                        continue;
                    }

                    const barHeight = b.high - b.low;

                    for (let r = startRow; r <= endRow; r++) {
                        const rowBottom = min + r * rowSize;
                        const rowTop = rowBottom + rowSize;
                        const overlapBottom = Math.max(b.low, rowBottom);
                        const overlapTop = Math.min(b.high, rowTop);
                        const overlap = overlapTop - overlapBottom;

                        if (overlap > 0) {
                            profile[r] += (overlap / barHeight) * b.volume;
                        }
                    }
                }
                ...
                while (currentVol < targetVol && (upIdx < numRows - 1 || downIdx > 0)) {
                    let upVol = 0;
                    if (upIdx + 1 < numRows) upVol += profile[upIdx + 1];
                    if (upIdx + 2 < numRows) upVol += profile[upIdx + 2];

                    let downVol = 0;
                    if (downIdx - 1 >= 0) downVol += profile[downIdx - 1];
                    if (downIdx - 2 >= 0) downVol += profile[downIdx - 2];

                    if (upVol === 0 && downVol === 0) break;

                    if (upVol >= downVol && upIdx < numRows - 1) {
                        if (upIdx + 1 < numRows) { upIdx++; currentVol += profile[upIdx]; }
                        if (currentVol < targetVol && upIdx + 1 < numRows) { upIdx++; currentVol += profile[upIdx]; }
                    } else if (downIdx > 0) {
                        if (downIdx - 1 >= 0) { downIdx--; currentVol += profile[downIdx]; }
                        if (currentVol < targetVol && downIdx - 1 >= 0) { downIdx--; currentVol += profile[downIdx]; }
                    } else {
                        break;
                    }
                }

                const pocPrice = min + (pocRow + 0.5) * rowSize;
                const vahPrice = min + (upIdx + 1) * rowSize;
                const valPrice = min + downIdx * rowSize;

                return { poc: pocPrice, vah: vahPrice, val: valPrice };
            };
        }
    };
}
