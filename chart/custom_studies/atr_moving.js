// ATR Moving Custom Study
// Converted from PineScript to TradingView Charting Library format

function createATRMoving(PineJS) {
    return {
        name: "ATR Moving",
        metainfo: {
            _metainfoVersion: 51,
            id: "atr_moving@tv-basicstudies-1",
            name: "ATR Moving",
            description: "ATR Dynamic Trail driven by a selectable moving average",
            shortDescription: "ATR Moving",
        ...
        constructor: function () {
            ...
            this.main = function (context, inputCallback) {
                ...
                // Get input values
                const atr_period = this._input(0);
                const atr_mult = this._input(1);
                const ma_type = this._input(2);
                const ma_period = this._input(3);
                ...
                // ATR Moving always transforms close into the selected MA first.
                // That MA becomes the source used by the trailing-stop logic.
                const src = close;
                ...
                switch (ma_type) {
                    ...
                    case "VWMA":
                        ...
                        if (this.vwma_buffer.length > ma_period) {
                    ...
                    case "LWMA":
                        ...
                        if (this.lwma_buffer.length > ma_period) {
                    ...
                    case "HULL":
                        ...
                        const half_length = Math.max(1, Math.floor(ma_period / 2));
                        const sqrt_length = Math.max(1, Math.floor(Math.sqrt(ma_period)));
                        ...
                        if (this.hull_wma2_buffer.length > ma_period) {
                    ...
                    case "WMA":
                        ...
                        if (this.wma_buffer.length > ma_period) {
                    ...
                    case "VWAP":
                        ...
                        if (this.vwap_buffer.length > ma_period) {
                    ...
                    case "ALMA":
                        ...
                        if (this.alma_buffer.length > ma_period) {
                            this.alma_buffer.shift();
                        }
                        
                        const m = Math.floor(alma_offset * (ma_period - 1));
                        const s = ma_period / alma_sigma;
                        let alma_norm = 0;
                        let alma_sum = 0;
                    ...
                    case "TEMA":
                        ...
                        const tema_alpha = 2.0 / (ma_period + 1);
                    ...
                    case "WWSMA":
                        ...
                        if (this.wwsma_buffer.length > ma_period) {
                        ...
                        if (this.wwsma_buffer.length < ma_period) {
                            ...
                            if (isNaN(this.ma_prev)) {
                                ...
                                trail1 = wwsma_sum / ma_period;
                            } else {
                                trail1 = (this.ma_prev * (ma_period - 1) + src) / ma_period;
                            }
                        }
                        break;
                    ...
                    case "ZLEMA":
                        ...
                        const zlema_lag = Math.floor((ma_period - 1) / 2);
                        const zlema_alpha = 2.0 / (ma_period + 1);
                    ...
                    case "LSMA":
                        ...
                        if (this.lsma_buffer.length > ma_period) {
                    ...
                    case "KAMA":
                        ...
                        if (this.kama_buffer.length > ma_period + 1) {
                            this.kama_buffer.shift();
                        }
                        
                        if (this.kama_buffer.length > ma_period) {
                    ...
                    case "VIDYA":
                        ...
                        if (this.vidya_cmo_buffer.length > ma_period + 1) {
                        ...
                        const vidya_alpha = 2.0 / (ma_period + 1) * cmo;
                    ...
                    case "SMMA":
                        ...
                        if (isNaN(this.smma_prev)) {
                            this.smma_sum += src;
                            this.smma_count++;
                            if (this.smma_count >= ma_period) {
                                trail1 = this.smma_sum / ma_period;
                                this.smma_prev = trail1;
                            } else {
                                trail1 = src;
                            }
                        } else {
                            trail1 = (this.smma_prev * (ma_period - 1) + src) / ma_period;
                            this.smma_prev = trail1;
                        }
                        break;
                    ...
                    case "McGinley":
                        ...
                        if (isNaN(this.mcginley_prev)) {
                            ...
                            const mg_divisor = ma_period * Math.pow(mg_factor, 4);
                    ...
                    case "SWMA":
                        ...
                        if (this.swma_buffer.length > ma_period) {
                            this.swma_buffer.shift();
                        }

                        let swma_sum = 0;
                        let swma_weight_sum = 0;
                        const swma_length = this.swma_buffer.length;
                        for (let i = 0; i < swma_length; i++) {
                            const weight = Math.min(i + 1, swma_length - i);
                            swma_sum += this.swma_buffer[i] * weight;
                            swma_weight_sum += weight;
                        }
                        trail1 = swma_weight_sum > 0 ? swma_sum / swma_weight_sum : src;
                        break;
                ...
                if (isNaN(this.atr_prev)) {
                    ...
                    atr = (this.atr_prev * (atr_period - 1) + tr) / atr_period;
                }
