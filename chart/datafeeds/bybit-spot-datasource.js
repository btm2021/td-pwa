/**
 * Bybit Spot Datasource Config
 */
const BYBIT_SPOT_CONFIG = {
    id: 'BYBIT_SPOT',
    name: 'Bybit Spot',
    ...
    logPrefix: '[Bybit Spot]',
    blacklistPatterns: [],
    maxSearchResults: 200,
    searchSuffixes: ['.BS', '.BYBITS']
};

/**
 * Bybit Spot Datasource
 * Implementation cho Bybit Spot Trading API
 */
class BybitSpotDatasource extends BaseDatasource {
    constructor(config = {}) {
        super(config);
        this.baseUrl = BYBIT_SPOT_CONFIG.baseUrl;
        this.wsUrl = BYBIT_SPOT_CONFIG.wsUrl;
    }

    getInfo() {
        return BYBIT_SPOT_CONFIG;
    }

    canHandle(symbolName) {
        const upper = symbolName.toUpperCase();
        return upper.startsWith('BYBIT_SPOT:') || upper.startsWith('BYBITS:');
    }
    ...
    async fetchExchangeInfo() {
        try {
            ...
            if (data.retCode !== 0 || !data.result || !data.result.list) {
                console.error(`${BYBIT_SPOT_CONFIG.logPrefix} API error:`, data.retMsg);
                return [];
            }
            ...
            console.error(`${BYBIT_SPOT_CONFIG.logPrefix} Error fetching instruments:`, error);
            return [];
        }
    }
    ...
    searchSymbols(userInput) {
        ...
        const results = filtered.map(s => {
            ...
            return {
                ...
                description: `${s.baseCoin}/${s.quoteCoin} Spot`,
                exchange: BYBIT_SPOT_CONFIG.exchange,
                type: BYBIT_SPOT_CONFIG.type,
                exchange_logo: BYBIT_SPOT_CONFIG.logo,
                logo_urls: [
                    `https://huobicfg.s3.amazonaws.com/currency_icon/${baseAsset}.png`,
                    BYBIT_SPOT_CONFIG.logo
                ],
                score: score
            };
        });
        ...
        return results
            ...
            .slice(0, BYBIT_SPOT_CONFIG.maxSearchResults)
            .map(r => {
                delete r.score;
                return r;
            });
    ...
    async resolveSymbol(symbolName) {
        ...
        try {
            ...
            return {
                ...
                supported_resolutions: BYBIT_SPOT_CONFIG.supported_resolutions,
                volume_precision: 2,
                data_status: 'streaming',
                full_name: `BYBIT_SPOT:${symbol}`,
                logo_urls: [
                    `https://huobicfg.s3.amazonaws.com/currency_icon/${baseAsset}.png`,
                    BYBIT_SPOT_CONFIG.logo
                ]
            ...
            console.error(`${BYBIT_SPOT_CONFIG.logPrefix} Error resolving symbol:`, error);
    ...
    async getSymbolPriceInfo(symbol) {
        try {
            ...
            console.error(`${BYBIT_SPOT_CONFIG.logPrefix} Error fetching price:`, error);
            return { minmov: 1, pricescale: 100 };
        }
    }
    ...
    async getBars(symbolInfo, resolution, periodParams) {
        ...
        try {
            const url = `${this.baseUrl}/market/kline?category=spot&symbol=${symbol}&interval=${interval}&start=${from * 1000}&end=${to * 1000}&limit=1000`;
            ...
            console.error(`${BYBIT_SPOT_CONFIG.logPrefix} Error fetching bars:`, error);
            throw error;
        }
    }
    ...
    subscribeBars(symbolInfo, resolution, onRealtimeCallback, subscriberUID) {
        ...
        ws.onerror = (error) => {
            console.error(`${BYBIT_SPOT_CONFIG.logPrefix} WebSocket error:`, error);
        };

        ws.onclose = () => {
            console.log(`${BYBIT_SPOT_CONFIG.logPrefix} WebSocket closed for`, symbol);
        };
