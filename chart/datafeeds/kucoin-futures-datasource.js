/**
 * KuCoin Futures Datasource Config
 */
const KUCOIN_FUTURES_CONFIG = {
    id: 'KUCOIN_FUTURES',
    name: 'KuCoin Futures',
    description: 'KuCoin Perpetual',
    exchange: 'KuCoin Futures',
    logo: 'image/iconexchange/kucoin.svg',
    supported_resolutions: ['1', '5', '15', '30', '60', '240', '1D', '1W', '1M'],
    type: 'crypto',
    baseUrl: 'https://autumn-heart-5bf8.trinhminhbao.workers.dev/?url=https://api-futures.kucoin.com/api/v1',
    wsUrl: 'wss://ws-api-futures.kucoin.com',
    logPrefix: '[KuCoin Futures]',
    blacklistPatterns: [],
    maxSearchResults: 200,
    searchSuffixes: ['.KF', '.KUCOINF']
};

/**
 * KuCoin Futures Datasource
 * Implementation cho KuCoin Perpetual API
 */
class KuCoinFuturesDatasource extends BaseDatasource {
    constructor(config = {}) {
        super(config);
        this.baseUrl = KUCOIN_FUTURES_CONFIG.baseUrl;
        this.wsUrl = KUCOIN_FUTURES_CONFIG.wsUrl;
    }

    getInfo() {
        return KUCOIN_FUTURES_CONFIG;
    }

    canHandle(symbolName) {
        const upper = symbolName.toUpperCase();
        return upper.startsWith('KUCOIN_FUTURES:') || upper.startsWith('KUCOINF:');
    }

    async fetchExchangeInfo() {
        try {
            const response = await fetch(`${this.baseUrl}/contracts/active`);
            const data = await response.json();

            if (data.code !== '200000' || !data.data) {
                console.error(`${KUCOIN_FUTURES_CONFIG.logPrefix} API error:`, data.msg);
                return [];
            }
            ...
            return data.data
                ...
                .map(s => ({
                    symbol: s.symbol.replace('USDTM', 'USDT'),
                    contractSymbol: s.symbol,
                    baseCurrency: s.baseCurrency,
                    quoteCurrency: 'USDT'
                }));
        } catch (error) {
            console.error(`${KUCOIN_FUTURES_CONFIG.logPrefix} Error fetching instruments:`, error);
            return [];
        }
    }
    ...
    searchSymbols(userInput) {
        ...
        const results = filtered.map(s => {
            ...
            return {
                symbol: s.symbol,
                full_name: `KUCOIN_FUTURES:${s.symbol}`,
                description: `${s.baseCurrency}/${s.quoteCurrency} Perp`,
                exchange: KUCOIN_FUTURES_CONFIG.exchange,
                type: KUCOIN_FUTURES_CONFIG.type,
                exchange_logo: KUCOIN_FUTURES_CONFIG.logo,
                logo_urls: [
                    `https://huobicfg.s3.amazonaws.com/currency_icon/${baseAsset}.png`,
                    KUCOIN_FUTURES_CONFIG.logo
                ],
                score: score
            };
        });
        ...
        return results
            ...
            .slice(0, KUCOIN_FUTURES_CONFIG.maxSearchResults)
            .map(r => {
                delete r.score;
                return r;
            });
    ...
    async resolveSymbol(symbolName) {
        ...
        try {
            ...
            return {
                ...
                supported_resolutions: KUCOIN_FUTURES_CONFIG.supported_resolutions,
                volume_precision: 2,
                data_status: 'streaming',
                full_name: `KUCOIN_FUTURES:${symbol}`,
                logo_urls: [
                    `https://huobicfg.s3.amazonaws.com/currency_icon/${baseAsset}.png`,
                    KUCOIN_FUTURES_CONFIG.logo
                ]
            ...
            console.error(`${KUCOIN_FUTURES_CONFIG.logPrefix} Error resolving symbol:`, error);
    ...
    async getSymbolPriceInfo(symbol) {
        try {
            const contractSymbol = symbol.replace('USDT', 'USDTM');
            ...
            console.error(`${KUCOIN_FUTURES_CONFIG.logPrefix} Error fetching price:`, error);
            return { minmov: 1, pricescale: 100 };
        }
    }
    ...
    async getBars(symbolInfo, resolution, periodParams) {
        ...
        const granularity = intervalMap[resolution] || '15';

        try {
            const url = `${this.baseUrl}/kline/query?symbol=${contractSymbol}&granularity=${granularity}&from=${from * 1000}&to=${to * 1000}`;
            ...
            console.error(`${KUCOIN_FUTURES_CONFIG.logPrefix} Error fetching bars:`, error);
            throw error;
        }
    }
    ...
    subscribeBars(symbolInfo, resolution, onRealtimeCallback, subscriberUID) {
        ...
        fetch(`${this.baseUrl}/bullet-public`)
            ...
            .then(data => {
                if (data.code !== '200000' || !data.data) {
                    console.error(`${KUCOIN_FUTURES_CONFIG.logPrefix} Failed to get WS token`);
                    return;
                }
                ...
                ws.onopen = () => {
                    const subscribeMsg = {
                        id: Date.now(),
                        type: 'subscribe',
                        topic: `/contractMarket/candle:${contractSymbol}_${interval}`,
                        response: true
                    };
                ...
                ws.onerror = (error) => {
                    console.error(`${KUCOIN_FUTURES_CONFIG.logPrefix} WebSocket error:`, error);
                };

                ws.onclose = () => {
                    console.log(`${KUCOIN_FUTURES_CONFIG.logPrefix} WebSocket closed for`, symbol);
                };

                this.subscribers[subscriberUID] = ws;
            })
            .catch(error => {
                console.error(`${KUCOIN_FUTURES_CONFIG.logPrefix} Error getting WS token:`, error);
            });
    }
}
