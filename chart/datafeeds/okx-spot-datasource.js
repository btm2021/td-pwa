/**
 * OKX Spot Datasource Config
 */
const OKX_SPOT_CONFIG = {
    id: 'OKX_SPOT',
    name: 'OKX Spot',
    ...
    searchSuffixes: ['.OS', '.OKXS']
};

/**
 * OKX Spot Datasource
 * Implementation cho OKX Spot Trading API
 */
class OKXSpotDatasource extends BaseDatasource {
    constructor(config = {}) {
        super(config);
        this.baseUrl = OKX_SPOT_CONFIG.baseUrl;
        this.wsUrl = OKX_SPOT_CONFIG.wsUrl;
    }

    getInfo() {
        return OKX_SPOT_CONFIG;
    }

    canHandle(symbolName) {
        const upper = symbolName.toUpperCase();
        return upper.startsWith('OKX_SPOT:') || upper.startsWith('OKXS:');
    }
    ...
    async fetchExchangeInfo() {
        try {
            ...
            if (data.code !== '0' || !data.data) {
                console.error(`${OKX_SPOT_CONFIG.logPrefix} API error:`, data.msg);
                return [];
            }

            // Filter USDT spot pairs và loại bỏ blacklist
            return data.data
                .filter(s => {
            ...
            console.error(`${OKX_SPOT_CONFIG.logPrefix} Error fetching instruments:`, error);
            return [];
        }
    }
    ...
    searchSymbols(userInput) {
        ...
        const results = filtered.map(s => {
            ...
            return {
                symbol: s.symbol,
                full_name: `OKX_SPOT:${s.symbol}`,
                description: `${s.baseCcy}/${s.quoteCcy} Spot`,
                exchange: OKX_SPOT_CONFIG.exchange,
                type: OKX_SPOT_CONFIG.type,
                exchange_logo: OKX_SPOT_CONFIG.logo,
                logo_urls: [
                    `https://huobicfg.s3.amazonaws.com/currency_icon/${baseAsset}.png`,
                    OKX_SPOT_CONFIG.logo
                ],
                score: score
            };
        });
        ...
        return results
            ...
            .slice(0, OKX_SPOT_CONFIG.maxSearchResults)
            .map(r => {
                delete r.score;
                return r;
            });
    ...
    async resolveSymbol(symbolName) {
        ...
        try {
            ...
            return {
                ...
                supported_resolutions: OKX_SPOT_CONFIG.supported_resolutions,
                volume_precision: 2,
            ...
            console.error(`${OKX_SPOT_CONFIG.logPrefix} Error resolving symbol:`, error);
    ...
    async getSymbolPriceInfo(symbol) {
        try {
            ...
            console.error(`${OKX_SPOT_CONFIG.logPrefix} Error fetching price:`, error);
            return { minmov: 1, pricescale: 100 };
        }
    }
    ...
    async getBars(symbolInfo, resolution, periodParams) {
        ...
        try {
            ...
            console.error(`${OKX_SPOT_CONFIG.logPrefix} Error fetching bars:`, error);
            throw error;
        }
    }
    ...
    subscribeBars(symbolInfo, resolution, onRealtimeCallback, subscriberUID) {
        ...
        ws.onerror = (error) => {
            console.error(`${OKX_SPOT_CONFIG.logPrefix} WebSocket error:`, error);
        };

        ws.onclose = () => {
            console.log(`${OKX_SPOT_CONFIG.logPrefix} WebSocket closed for`, symbol);
        };
